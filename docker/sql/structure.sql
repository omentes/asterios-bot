...
--
-- Table structure for table `version_notification`
--
...
constraint notifications_chat_id_type_uindex
    unique (chat_id, type)
...
create index event_created_index
    on event (created);
...
--
-- Indexes for table `new_raids`
--
ALTER TABLE `new_raids`
    ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `new_raids_title_timestamp_server_uindex` (`title`,`timestamp`,`server`),
  ADD KEY `new_raids_created_at_index` (`created_at`);
...
--
-- AUTO_INCREMENT for table `new_raids`
--
ALTER TABLE `new_raids`
